...
vws = [0.5:0.01:1];
vwp = [0.5:0.01:1];
for i = 1:100 % 100 repetitions starting from different intitial values
    ...
    wp0 = rpwp(1);
    [par(:,i),fval(:,i)] = fmincon(@(x)MeanSquaredErrorCI(expmatrix1,expmatrix2,expmatrix3,expmatrixTilted,x),[IB0,Instr0,CueStr0,CueWeak0,ws0,wp0],[],[],[],[],[-5,-5,-5,-5,0.5,0.5],[5,5,5,5,1,1]);
end
[u,v]=min(fval);
